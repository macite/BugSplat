...
//---------------------------------------------------------------------------
  ...
  function CreateSprite(layerName, animationName: String): Sprite; overload;
  ...
  function CreateSprite(layerName, animationName: String; evts: SpriteEventHandler): Sprite; overload;
...
//---------------------------------------------------------------------------
  ...
  procedure SpriteMoveTo(s: Sprite; const pt: Point2D; inSeconds: Longint);
...
//=============================================================================
  ...
  function CreateSprite(layerName, animationName: String): Sprite; overload;
  begin
    result := CreateSprite(BitmapNamed(layerName), AnimationScriptNamed(animationName));
  end;
  ...
  begin
    result := CreateSprite(BitmapNamed(layerName), AnimationScriptNamed(animationName));
    if assigned(result) then result^.evts := evts;
  end;
  ...
  begin
    ...
    if assigned(s) and assigned(s^.evts) then
    begin
  ...
  var
    ...
    trans: Matrix2D;
  begin
    if not Assigned(s) then begin RaiseWarning('No sprite supplied'); exit; end;
    ...
    begin
      ...
      begin
        ...
        if Assigned(s^.evts) then SpriteEventHandler(s^.evts)(s, 'Arrived');
      end;
    end;
  end;
...
//---------------------------------------------------------------------------
  ...
  begin
    ...
    s^.destination := pt;
    s^.arriveInSec := inSeconds;
    s^.isMoving := true;
    s^.movingVec := VectorMultiply(UnitVector(VectorFromPoints(CenterPoint(s), pt)), PointPointDistance(CenterPoint(s), pt) / inSeconds);
    s^.lastUpdate := TimerTicks(_spriteTimer);
  end;
